"""
ES Index Sync Script (harvestedfn2_fin) - Optimized Version
- Keyset pagination instead of OFFSET (no performance degradation on large tables)
- Larger ES bulk chunk_size
- Disable refresh during bulk import

Usage:
    python sync_es_index_fin_v2.py --action=info      # View index info
    python sync_es_index_fin_v2.py --action=create    # Create new index and sync data
    python sync_es_index_fin_v2.py --action=rebuild   # Delete old index and rebuild
    python sync_es_index_fin_v2.py --action=sync      # Sync data only (index must exist)
"""
...
# Index name
INDEX_NAME = "mrservice_harvestedfn2_fin_v2_test"
...
INDEX_MAPPING = {
    "settings": {
        ...
        "index": {
            ...
            "refresh_interval": "-1"  # Disable refresh during bulk import
        },
        "analysis": {
            "filter": {
                "phonetic_filter": {
    ...
    "mappings": {
        "properties": {
            ...
            "ValidGenus": text_keyword_phonetic_field(),
            "ValidFamily": text_keyword_phonetic_field(),
...
def main():
    parser = argparse.ArgumentParser(description="ES Index Sync Tool (harvestedfn2_fin) - Optimized")
    parser.add_argument(
        "--action",
        choices=["create", "rebuild", "sync", "info"],
        required=True,
        help="create | rebuild | sync | info"
    )
